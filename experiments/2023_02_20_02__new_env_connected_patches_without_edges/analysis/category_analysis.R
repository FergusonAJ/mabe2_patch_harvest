...
ggplot(df_summary, aes(x = task_quality_mean, fill = as.factor(category))) + 
  geom_histogram(binwidth=0.025) +
  scale_fill_manual(values = category_color_map) +
  scale_x_continuous(limits = c(0,1)) +
  facet_grid(rows = vars(as.factor(category)))
ggsave(paste0(plot_dir, '/category__task_quality_histrograms.png'), units = 'in', width = 8, height = 6)
